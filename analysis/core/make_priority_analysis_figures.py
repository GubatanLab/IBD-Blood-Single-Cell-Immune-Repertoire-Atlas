...
cnt=shared.groupby("Diagnosis").lineage_id.nunique().reindex(dxs,fill_value=0);axs[1,1].bar(dxs,cnt.values,color=[colors[x] for x in dxs]);axs[1,1].set_ylabel("Shared IgA- and IgG-plasma-state lineages");axs[1,1].set_title("Cross-state lineage occupancy is rare");axs[1,1].text(.02,.94,"Terminal-branch identity cannot be assigned\nwithout cell-linked full heavy sequences",transform=axs[1,1].transAxes,va="top",fontsize=7,color="#333",bbox={"facecolor":"white","edgecolor":"none","alpha":.82,"pad":1.5});panel(axs[1,1],"D")
