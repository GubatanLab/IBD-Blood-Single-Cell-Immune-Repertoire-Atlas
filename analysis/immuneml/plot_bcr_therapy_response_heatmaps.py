...
METRICS = {
    "roc_auc": ("TherapyResponse_roc_auc_mean", "ROC AUC", 0.5, 1.0, "viridis"),
    "balanced_accuracy": ("TherapyResponse_balanced_accuracy_mean", "Balanced Accuracy", 0.45, 0.80, "magma"),
    "mcc": ("TherapyResponse_matthews_corrcoef_mean", "Matthews Correlation", -0.10, 0.65, "coolwarm"),
    "f1": ("TherapyResponse_f1_mean", "F1 Score", 0.35, 0.90, "cividis"),
}
...
def top_model_labels(df: pd.DataFrame, top_n: int) -> list[str]:
    ...
    ranking = (
        work.groupby("row_label", sort=False)["TherapyResponse_roc_auc_mean"]
        .mean()
...
def save_individual_heatmaps(df: pd.DataFrame, output_dir: Path, row_order: list[str]) -> None:
    for metric_name, (metric_col, title, vmin, vmax, cmap) in METRICS.items():
        ...
        for ext in ("png", "pdf", "svg"):
            fig.savefig(output_dir / f"bcr_therapy_response_{metric_name}_heatmap.{ext}", dpi=600, bbox_inches="tight")
        plt.close(fig)


def save_combined_heatmap(df: pd.DataFrame, output_dir: Path, row_order: list[str], filename_suffix: str = "") -> None:
    fig_height = max(8.8, 2.8 + 0.25 * len(row_order))
    fig, axes = plt.subplots(1, 4, figsize=(23.0, fig_height), gridspec_kw={"wspace": 0.58})
    ...
    fig.suptitle("immuneML BCR Repertoire Therapy Response Models", fontsize=16, y=0.995)
    fig.subplots_adjust(left=0.075, right=0.985, top=0.88, bottom=0.12, wspace=0.58)
    for ext in ("png", "pdf", "svg"):
        fig.savefig(output_dir / f"bcr_therapy_response_prediction_metric_heatmaps{filename_suffix}.{ext}", dpi=600, bbox_inches="tight")
        fig.savefig(output_dir / f"bcr_therapy_response_prediction_metric_heatmaps_tcr_style{filename_suffix}.{ext}", dpi=600, bbox_inches="tight")
    plt.close(fig)


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--aggregate", default="chain_bcr_immuneml/bcr_therapy_response_lr_svm_feature_model_summary_aggregate.csv")
    ...
    print(f"Wrote therapy response heatmaps to {output_dir}")
