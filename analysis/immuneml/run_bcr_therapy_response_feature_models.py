...
COMPARISONS = {
    "combined_biologic_nonresponder_vs_responder": ("AntiTNF", "Ustekinumab", "Vedolizumab"),
    "anti_tnf_nonresponder_vs_responder": ("AntiTNF",),
    "ustekinumab_nonresponder_vs_responder": ("Ustekinumab",),
    "vedolizumab_nonresponder_vs_responder": ("Vedolizumab",),
}
RESPONSE_LABELS = {"Inflamed": "NonResponder", "Noninflamed": "Responder"}
POSITIVE_LABEL = "NonResponder"
...
def comparison_metadata(input_root: Path, chain_group: str, comparison: str) -> pd.DataFrame:
    biologics = COMPARISONS[comparison]
    ...
    metadata["TherapyResponse"] = metadata["Inflammation1"].map(RESPONSE_LABELS)
...
def load_dataset(
    ...
    for row in metadata.itertuples(index=False):
        ...
        labels.append(getattr(row, "TherapyResponse"))
...
def load_isotype_dataset(
    ...
    labels = merged["TherapyResponse"].to_numpy()
...
def main() -> None:
    ...
    parser.add_argument("--output", default="chain_bcr_immuneml/bcr_therapy_response_feature_model_summary.csv")
